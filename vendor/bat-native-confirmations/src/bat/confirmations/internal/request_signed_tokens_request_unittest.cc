...
class BatConfirmationsRequestSignedTokensRequestTest : public ::testing::Test {
 protected:
  std::unique_ptr<RequestSignedTokensRequest> request_;

  BatConfirmationsRequestSignedTokensRequestTest() :
      request_(std::make_unique<RequestSignedTokensRequest>()) {
    // You can do set-up work for each test here
  }

  ~BatConfirmationsRequestSignedTokensRequestTest() override {
  ...
  std::vector<Token> GetTokens(
    ...
    // Blinded tokens used for the above tokens:
    // "blindedTokens": [
...
TEST_F(BatConfirmationsRequestSignedTokensRequestTest,
    BuildUrl) {
  // Arrange
  ...
  // Assert
  const std::string expected_url = "https://ads-serve.bravesoftware.com/v1/confirmation/token/d4ed0af0-bfa9-464b-abd7-67b29d891b8b";  // NOLINT
...
TEST_F(BatConfirmationsRequestSignedTokensRequestTest,
  ...
  // Assert
  const std::string expected_header_value = R"(keyId="primary",algorithm="ed25519",headers="digest",signature="m5CxS9uqI7DbZ5UDo51bcLRP2awqcUSU8tfc4t/ysrH47B8OJUG1roQyi6/pjSZj9VJuj296v77c/lxBlCn2DA==")";  // NOLINT
  EXPECT_EQ(expected_header_value, header_value);
}
