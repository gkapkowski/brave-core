...
class BatConfirmationsSecurityUtilsTest : public ::testing::Test {
 protected:
  BatConfirmationsSecurityUtilsTest() {
    // You can do set-up work for each test here
  }

  ~BatConfirmationsSecurityUtilsTest() override {
...
TEST_F(BatConfirmationsSecurityUtilsTest,
    Sign) {
  // Arrange
  ...
  // Act
  const std::string signature = Sign(headers, key_id, private_key);
...
TEST_F(BatConfirmationsSecurityUtilsTest,
  ...
  // Act
  const std::vector<Token> tokens = GenerateTokens(5);
...
TEST_F(BatConfirmationsSecurityUtilsTest,
    BlindTokens) {
  // Arrange
  const std::vector<Token> tokens = GenerateTokens(7);
...
TEST_F(BatConfirmationsSecurityUtilsTest,
  ...
  // Act
  const std::vector<uint8_t> sha256 = Sha256(body);
  const std::string sha256_base64 = Base64Encode(sha256);
